...
def _parse_fixed_width_obs(raw: str, num_data: int, obs_info, obs_info_code,
    ...
    """Classic RINEX2 fixed-width (16-char fields)."""
    raw = raw.ljust(16 * num_data)
    for obs_count in range(num_data):
        if obs_info[obs_count] is None:
            continue

        field = raw[16*obs_count:16*obs_count+16]
        valstr = field[:14]
        lli_ch = field[14:15]
        ssi_ch = field[15:16]
        ...
        if val == 0.0:
            ...
            obs_data[time_index, sat_index, obs_info[obs_count]] = val

        # LLI (if present)
        if lli_ch and (not lli_ch.isspace()) and lli_ch != '0':
            try:
                if (int(lli_ch) & 0x3) != 0:
                    obs_data[time_index, sat_index, LOSS_OF_LOCK] = 1
            except ValueError:
        ...
        # SSI (if present)
        if ssi_ch and (not ssi_ch.isspace()) and ssi_ch != '0':
            try:
                signal_strength_flag = int(ssi_ch)
...
"""
    ...
    try:
        ...
        # Interval handling
        FORCE_STANDARD_30S = False  # Set True only if downstream code *requires* 30s
        STANDARD_INTERVAL = 30.0 * DAYS_PER_SECOND

        interval = STANDARD_INTERVAL  # default if INTERVAL missing
        interval_undersampled = 0 # for now assume not undersampled
        ...
        while(1):
            ...
            # Approx Position XYZ Block
                ...
                # If DELTA H/E/N was seen before XYZ, apply it now
                if pending_apply_antenna_offset and antenna_offset is not None:
            ...
            # Antenna Processing Block
            elif(line[60:80] == "ANTENNA: DELTA H/E/N"):
                antenna_offset = list(map(float, line.strip().split(None, 3)[0:3]))
                if not XYZ_set:
                    # Septentrio sometimes puts DELTA before XYZ; apply later
                    pending_apply_antenna_offset = True
                else:
                    # Apply immediately if XYZ already known
                    offset = antenna_offset
                    r = XYZ[3]
                    r_ratio = (r + offset[0]) / r
                    r = r + offset[0]
                ...
                if FORCE_STANDARD_30S:
                    if data_interval != STANDARD_INTERVAL:
                        warnings.warn(
                            "RINEX file '%s' has nonstandard data interval of %.1f seconds, forcing to standard interval of %.1f seconds"
                            % (filename, data_interval*SECONDS_PER_DAY, STANDARD_INTERVAL*SECONDS_PER_DAY)
                        )
                        interval = STANDARD_INTERVAL
                        if(data_interval > 1.99 * interval):
                            interval_undersampled = int(data_interval/interval)-1
                else:
                    # Respect the file interval (needed for 10s/1s files)
...
def estimate_and_apply_local_code_bias(MJD, Sat_array, obs_data, station_code,
    ...
    lam1 = SPEED_OF_LIGHT / nu_L1_GPS
    lam2 = SPEED_OF_LIGHT / nu_L2_GPS

    num_obs = obs_data.shape[0]
    bias_sec_by_sat = {}

    # GPS only (0..99). If you need other constellations, extend carefully with their freqs.
    for sat in range(100):
        ...
        for i in range(num_obs):
            ...
            L1 = obs_data[i, s, L1_pos]
            L2 = obs_data[i, s, L2_pos]
            if (L1 == BAD_DATA_CODE) or (L2 == BAD_DATA_CODE):
                continue

            P4 = P2 - P1                  # meters
            L4 = (L1 * lam1) - (L2 * lam2)  # meters
            diffs.append(P4 - L4)

        if len(diffs) < min_samples:
            continue

        bias_m = float(np.median(diffs))
        bias_sec = bias_m / SPEED_OF_LIGHT
        bias_sec_by_sat[sat] = bias_sec

        # Apply correction by shifting P2 only:
        # P4_corr = (P2 - bias_m) - P1 = P4 - bias_m
        for i in range(num_obs):
            s = Sat_array[i, sat]
            if s < 0:
                continue
            if obs_data[i, s, P2_pos] != BAD_DATA_CODE:
                obs_data[i, s, P2_pos] -= bias_m

    return bias_sec_by_sat
...
def get_station_base_file(MJD, station_code, output_directory=".", overwrite=0,
                          local_rinex_obs_file=None):
    # --- NEW: allow caller to bypass web download and use an existing local RINEX obs file
    if local_rinex_obs_file is not None:
        if not os.path.isfile(local_rinex_obs_file):
            raise Albus_RINEX.No_RINEX_File_Error(
                "Local RINEX obs file not found: '%s'" % local_rinex_obs_file
            )
        return local_rinex_obs_file
...
obs_data     O  The output data array, as
                ...
                you get data from
                obs_data[i,Sat_array[i,s], x]
                where x is the index of sthe data kind you want, as specified
                in the _DATA_POS dict
...
################################################################################
    ...
    for m in range(MJD_start, MJD_end):
        ...
        # --- NEW: use a provided local RINEX obs file instead of downloading
        if local_rinex_obs_file is not None:
            data_file = local_rinex_obs_file
            if not os.path.isfile(data_file):
                raise Albus_RINEX.No_RINEX_File_Error(
                    "Local RINEX obs file not found: '%s'" % data_file
                )
        else:
...
def get_station_base_observations_with_bin(MJD_start,
    ...
    sta_MJD, Sat_array, obs_data, sta_XYZ, sta_bias_valid, sat_block_pos = \
            get_station_base_observations(MJD_start,
                              ...
                              overwrite,
                              raise_bias_error,
                              local_rinex_obs_file=local_rinex_obs_file,
                              use_external_dcb_files=use_external_dcb_files,
                              estimate_local_dcb_if_missing=estimate_local_dcb_if_missing
                              )
             #get_station_base_observations(MJD_start,
                                           #MJD_end,
                                           #station_code,
